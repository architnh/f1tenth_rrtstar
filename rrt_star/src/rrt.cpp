               ...
               // float dist_to_goal = sqrt( pow(final_path_output[q].x -0,2) +
                ...
                if(distance < (l_value * 0.33)){//} && dist_to_goal < dist_from_goal_to_car){
                    ...
                    for(int r=0;r<final_path_output.size();r++){
                        distance_over_1=sqrt( pow(final_path_output[final_path_output.size()-r-1].x -0,2) +
                        pow(final_path_output[final_path_output.size()-r-1].y - 0,2));
                    

                        if(distance_over_1 > (l_value * 0.33)){
                            r_return = final_path_output.size()-r-1;
                            break;
                        }
                    }
                    float min_dist = 1000000;
                    float calc_point_distance;
                    float final_x;
                    float final_y;
                    for(int h=0;h<100;h++){
                        float x_val = final_path_output[q].x + ( (h/100) * abs(final_path_output[q].x - final_path_output[r_return].x));
                        float y_val = final_path_output[q].y + ( (h/100) * abs(final_path_output[q].y - final_path_output[r_return].y));

                        calc_point_distance = min_dist > sqrt(pow( x_val-0,2) + pow(y_val- 0,2)); 
                        if(min_dist > calc_point_distance && l_value * 0.33 < calc_point_distance){
                            min_dist = calc_point_distance;
                            final_x=x_val;
                            final_y=y_val;
                        }
